contractsContainer = {};
ctrAddressesContainer = {};

contractsContainer["AlphaLayer"] = {
...
contractsContainer["AlphaLayer"].contractClass = web3.eth.contract(contractsContainer["AlphaLayer"].interface);
contractsContainer["AlphaLayer"].contract = contractsContainer["AlphaLayer"].contractClass.at(contractsContainer["AlphaLayer"].address);
if (!ctrAddressesContainer["AlphaLayer"]){ ctrAddressesContainer["AlphaLayer"] = [] }ctrAddressesContainer["AlphaLayer"].push("0xb03E6fc8627BFEf96efE0B1A41d79d019D38c5E6");
...
contractsContainer['AlphaLayer'].contract.readString(addr,layer,function(err,data){
...
contractsContainer['AlphaLayer'].contract.readUint(addr,layer,function(err,data){
if(err){console.log("Error: "+err);}else{
$(ob).text(escapeX(data[0]));
}
});
...
contractsContainer['AlphaLayer'].contract.readBool(addr,layer,function(err,data){
if(err){console.log("Error: "+err);}else{
$(ob).text(escapeX(data[0]));
}
});
...
contractsContainer['AlphaLayer'].contract.readAddress(addr,layer,function(err,data){
if(err){console.log("Error: "+err);}else{
$(ob).text(escapeX(data[0]));
}
});
